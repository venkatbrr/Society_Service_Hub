-- Fix: Drop the old 3-parameter overload of get_community_visits
-- PostgreSQL treats functions with different parameter counts as separate functions.
-- The previous migration created a NEW 4-param function while the old 3-param one still exists.
-- PostgREST cannot resolve overloaded functions, causing silent failures.

-- Drop the old 3-parameter version
DROP FUNCTION IF EXISTS get_community_visits(UUID, UUID, TEXT);

-- Recreate the single version with 4 parameters (p_time_scope has a default)
