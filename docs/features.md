...
| **Payer mode** | Contribution mode opens on a segmented **General · Other · Sponsor** control (Sponsor shown to leads and platform admins only; hidden entirely when editing an existing row, whose mode is fixed by what it already is). Labels are deliberately short so all three fit one row on a phone — the notice card above and the field label below carry the full meaning. |
| **Other contribution** | For money handed over for something specific — food, the god idol, prasadam — by someone the collector identifies by name rather than by flat. Fields: contributor name (required), flat (optional, free text), **Contributing for** (required, free text, with one-tap suggestion chips from `SUGGESTED_PURPOSE_LABELS`), plus the shared amount / method / notes. It deliberately picks **no flat**, so it never marks a flat as paid and never collides with the one-contribution-per-flat rule; the flat field is a note about where the money came from. Any collector, treasurer, or lead may record one, and no block prompt appears. |
| **Outside sponsor** | Sponsor mode replaces the resident picker with a sponsor name (required), phone, and note, and does not prompt for the caller's block. A sponsor row shows on the Contributions list as "Outside sponsor" and can only be reopened for editing by a lead — collectors and treasurers see it as read-only. |
