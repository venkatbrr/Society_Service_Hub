...
| **Rules** | Disabling blocks removes active scoping for residents and in-charges but preserves historical contributions. Per-block resident and in-charge counts are surfaced to support safe archival. **Re-adding a previously archived block name restores that block** rather than failing on a duplicate name. All labels come from `blockLabel` in AuthContext. **The fetched block list renders whenever it is non-empty, independent of the enabled toggle** — the toggle only gates the Add/Rename/Archive controls, so existing blocks stay visible even while `AuthContext`'s `blocksEnabled` is still loading (that value resolves in a second, non-blocking phase — see the known trap in `CLAUDE.md` §9). Confirmations use `confirmAction` (platform-split), not `Alert.alert`. |
