...
| Aspect | Details |
|--------|---------|
| **Purpose** | Enable food businesses and home chefs to post scheduled, time-gated food drops (for example weekend specials closing the previous evening) with cut-off deadlines. |
...
| **Business rules** | Owners publish a drop specifying title, prep notes, delivery date, delivery time (selected from system time picker), cut-off date/time deadline, and items offered (name, unit, price). Delivery time must be strictly later than the pre-order cut-off timestamp. Cut-off deadlines automatically block new resident orders when passed. If `max_orders` is set, it is treated as a max total items cap for the drop (sum of quantities across all active pre-orders), and the resident flow blocks submissions that would exceed that cap. Residents place pre-orders before the deadline with flat number and contact phone. Food drop tiles show host identity (name plus flat number when available), side-by-side close and delivery timing chips, and current ordered-item totals against the configured max cap. The owner management dashboard (`app/network/drops/manage/[id].tsx`) automatically aggregates item totals across all pre-orders for kitchen preparation and provides a resident delivery roster with Call & WhatsApp shortcuts. Owners mark resident orders as `fulfilled` upon delivery and mark the drop `completed`. |
| **Navigation** | From MCN hub ("Food Pre-Orders & Flash Drops" tile), `/network/drops`, `/network/drops/add`, `/network/drops/[id]`, and `/network/drops/manage/[id]`. On web deploys, deep links for dynamic drop and manage routes are rewritten through query-param bridge paths to preserve refresh support. |
| **Roles** | Anonymous users and residents can browse food drops. Login is required to place/cancel pre-orders or publish/manage drops. Drop creators can close orders early, manage orders, mark orders fulfilled, and complete drops. |
