...
| **Catalog rules** | Tabs: Active / Closed / My drops, ordered by `cutoff_at` ascending. Signed-in users additionally get per-drop order counts, item quantities, and revenue metrics (total, completed, order count), computed client-side while excluding cancelled orders. Anonymous viewers see only `status = 'open'` drops. Cards show host identity (name plus flat number when available), the linked business listing when present, and side-by-side close/delivery timing chips. The cover photo is **30% of viewport height** (`getNetworkTileImageHeight()`, clamped 150–280px, re-measured from `useWindowDimensions()`) rather than the old fixed 108px strip — food is bought from the photo, and a thin band above the text did not sell it. The crop is `contentPosition="top"`, since `contentFit="cover"` centre-crops and takes the top off a plated dish. Community business cards use the same token for the same reason. A `?id=` param redirects into the drop detail (web deep-link bridge). |
...
| **Presentation** | The list uses a poster card: full-width cover at 30% of viewport height (`getNetworkTileImageHeight`), top-anchored crop, an overlaid day/month badge, a "Today"/"Tomorrow" pill on the two days that matter, serif title, and the register-by date as a chip. The previous 96px thumbnail row read as a settings list rather than a what's-on feed. Category chips render **only for categories the community has actually posted in**, and the row is hidden entirely below two — six permanent chips made an empty screen look like a filter problem. Selecting a category that disappears on a scope switch falls back to "All". The detail screen's hero photo is tappable and opens `ImageViewer` full-screen, with a "Tap to view" hint, because the cover is cropped. |
