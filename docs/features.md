...
| **Invite-link prefill survives signed-out signup** | The screen pre-fills `code` from `?code=` (`useLocalSearchParams`), but a signed-out visitor gets bounced to `/login` first, and `usePathname()` never carries the query string. `lib/inviteCode.ts` parks the code (sessionStorage on web, module value on native) the moment `app/_layout.tsx`'s auth guard redirects an unauthenticated visit away, and the guard re-appends it to the `/community-select` redirect once the new account has no community yet — so it survives a full-page Google OAuth redirect. The screen reads the parked code as a fallback when the query string is missing, and is the only place that clears it. Both invite entry points (Providers header and the Community tab's code panel) build the same `/api/share-community?id=…&code=…` link, so they share this path exactly. See `architecture.md` §2 for why the guard peeks rather than consumes. |
...
Both the `pending` and `rejected` states offer **"Join existing community"**, which leaves for `/community-select`. Requesting a new community is the easiest onboarding step to pick by mistake, and the requester should not have to wait out a review — or a rejection — to enter the community they actually belong to. See `architecture.md` §2 for the routing exemption that lets the button work.
...
| Aspect | Details |
|--------|---------|
| **Rules** | **Providers carrying feedback sort to the top of whatever is on screen**, filtered category included: rated providers first, then ones with a written description, then bare name-and-number contacts (`rankProvidersByFeedback` in `app/(tabs)/index.tsx`). Most directory entries are just a phone number, so an entry a neighbour actually vouched for would otherwise sit buried in an alphabetical run. The query still asks for `avg_rating` descending then `name` ascending, and the ranking sort is stable, so that ordering survives inside each tier; it runs client-side because PostgREST can only `.order()` on plain columns and "has a description" is an expression, and the fetch pulls the whole community in one page (limit 500) so it never ranks a partial set. Two-level filter: a group row (All Services, Home Support, Repairs & Maintenance, Healthcare & Wellness, Personal Care, Transport & Vehicle Care, Events & Occasions, Education & Coaching, Government & Docs, Other) then a category chip row scoped to the active group. Selecting a group applies `.in('category', groupCategories)`; selecting a category applies `.eq('category', …)`. Groups come from `CATEGORY_GROUPS` in `constants/categories.ts`. Search matches name, category, and phone number (digits-stripped on both sides) using placeholder `"Search by name or phone number..."`. A network or load error surfaces a connection error view with pull-to-refresh retry. An **"Available now"** chip filters the loaded list client-side to Maid/Cook providers currently inside a reported free time band (see Maid/Cook availability below). |
...
| **Cards** | In order: Pre-order Food → `/mcn/drops` (open menus **not yet past their cutoff**) · Community Business → `/mcn/business` (active listings) · Parent Corner → `/mcn/parents` (children listed) · Community Carpooling → `/mcn/carpools` (active rides). **Split on 2026-08-24**: menus and businesses shared one merged card titled "Pre-order Food & Community Business" that opened `/mcn/drops`, and `/mcn/business` was reachable *only* from a segmented switcher inside that screen — a whole section a level deeper than its peers, behind a title that named two things and opened one. The switcher is gone from both screens with the split; the hub is now the only route between them. |
| **Glyph motion** | Each card's icon carries a slow idle animation matched to its subject (the bag sways, the shutter lifts, the car drifts, the group breathes), on deliberately unequal cycles so the screen never pulses in unison — `components/AnimatedTileGlyph.tsx`. Static under reduce-motion. Spec in [`verandah.md`](verandah.md) §Idle motion. |
...
| Aspect | Details |
|--------|---------|
| **Purpose** | Directory of resident-run businesses, plus businesses the society itself runs |
| **Tables** | `mcn_listings`, `mcn_business_categories`, `mcn_products`, `ratings`, `profiles` |
| **Rules** | Community-scoped with 300 ms debounced name search. A horizontal category chip bar filters by `category_id`; tapping the active chip returns to All. Listings are grouped by category into collapsible sections **ordered alphabetically by category name** (Beauty & Personal Care first), with the two catch-alls — the `Other` lookup category and the fallback bucket for a listing whose category row is missing — sunk below the named ones, and Inactive Businesses last. Sections previously fell out in whatever order the rows came back, which tracked each category's newest listing and so reshuffled the page every time somebody posted. The **filter chip row keeps its curated `sort_order`** (Food & Beverages first) — it is a shortcut to the common categories, not a list to scan. Within a section, active listings sort ahead of paused ones and inactive listings stay visible behind an inactive badge. Cards show summary only (cover image, owner, category badge) — offerings and prices appear after opening the listing. The cover shows the **top 30% of the photo's own height** via `getTopCropTileImageStyle()` — top-anchored, bounded below by `getNetworkTileImageHeight()` and above by the hero height — matching food drops; the listing detail hero is 30% via `getMediaHeroHeight()`. See §4.3 for why. A card for a **community-run** listing (`is_community_business`) shows "Run by the community" where a resident's listing shows owner name · flat, avatars off the business name instead of the owner's, and shares with that line in place of "Owner: …". Each card's share action links to `/api/share-listing?id=…` (`api/share-listing.ts`), the same OG-preview pattern as food drops. `mcn_listings` has no anon-readable SELECT policy, so the endpoint reads through `get_listing_og_card(p_id)`, a `SECURITY DEFINER` RPC exposing only `name`/`description`/`image_url` — never a direct table read. |
| **Anti-spam rules** | All enforced server-side by triggers on `mcn_listings` (migrations `20260819000000`, `20260821000000`), not just the UI: **one listing per resident per category** (create a second under the same category and it's rejected — edit the existing one instead); **max 5 active listings per resident** across all categories (reactivating a 6th is blocked until another is paused/deleted); **1 new listing per resident per 24 hours**. Each check raises a plain-language error the client surfaces directly (`error.message`), so no separate client-side copy to keep in sync. **Community-run listings are exempt from all three** (`20260925000000`) and do not count toward a lead's own quota — a society can have several, and listing them is not the behaviour these limits exist to stop. |
...
| **Roles** | All residents view and create. **Only a president or vice president can mark a listing "Run by the community"** — the toggle is hidden from everyone else, and the server rejects the flag from a non-lead regardless of the UI. Owner or lead can manage and delete. Delete sits in a **bottom-of-screen danger zone** on the manage screen (`components/DangerZone.tsx`, shared with food drops) — it names the listing and its products as the loss, points the host at pausing instead if they are only closing for a while, and carries the shared spam caution. |

### Add listing — `app/mcn/listing-add.tsx`

Business name required (max 80). Category required, from the `mcn_business_categories` lookup. Description optional (max 280). Contact phone required, normalized to 10 digits.

Presidents and vice presidents also see a **Run by the community** switch above the items section. It is for a business the society runs itself — the community pharmacy, the society store — which has no flat behind it: the listing is credited to the community rather than to the lead who filed it, and the per-resident listing limits do not apply. The switch is not rendered for other residents, and the flag is rejected server-side if sent anyway.

A **Products & services** section sits on this same screen, below the phone field (2026-08-24) — items used to be reachable only from the manage screen after saving, so residents did not know to add them. With nothing staged, the section renders as a single accent-tinted tappable card ("Add your first item") rather than a muted hint — the whole block is the call to action, since a small outlined button here was too easy to scroll past. Once an item exists it becomes the staged list with a filled "Add item" button in the section header. Either entry point opens the same item editor the manage screen uses (type toggle, photo, name, optional price, unit, description), but staged items are held in local state with no `mcn_products` row yet; each can be edited or removed before saving. Adding items stays optional. "Create listing" inserts the listing, then bulk-inserts the staged items with `is_available: true` and `sort_order` following the on-screen order. Because the listing row already exists at that point, a failed item insert is reported on its own ("Listing created, items could not be saved") instead of as a failed listing. Navigates to the manage screen on success either way.
...
| **Rules** | **Browse-and-contact only — there is no in-app ordering.** Offerings split by `item_type` into Products and Services, each row showing name, optional description, availability, and either `₹ amount / unit` or **"Price on request"** when `price IS NULL`. Unavailable offerings show a "Not available" badge. A community-run listing shows the business name and "Run by the community" in the owner card instead of a resident's name and flat, and the contact copy says "the business" rather than an owner's name. Non-owners see a line above the offerings — "Prices are indicative. Call or message *owner* to place an order." — and a labelled **Call / WhatsApp** pair below the description (via `buildWhatsAppUrl`); both are hidden when no contact number exists, and hidden from the owner **except on a community-run listing** — that number belongs to the society, not to the lead who filed the listing, so it stays visible to everyone including its `owner_id`. Also carries the ratings/reviews block and the report flow. |
...
| Aspect | Details |
|--------|---------|
| **Rules** | Toggle listing active/paused, edit details including category, and manage offerings. Leads additionally get the **Run by the community** switch inside the details form, so an existing listing can be converted either way. The offering modal supports `item_type` (`product`/`service`) and an optional price — blank stores `NULL` and renders as "Price on request". Deleting a product or a whole listing with orders in history is gracefully blocked with a prompt to pause instead (SQLSTATE 23503 handling). |
...
### 4.3 Pre-order menus — `app/mcn/drops/*`

Routes: `index` (catalog) · `add` · `[id]` (detail + ordering) · `manage/[id]` (host dashboard) · `manage/index`

**The feature is called "menu" on screen and `drop` in the code** (renamed from "food drop" on 2026-08-17). The FAB reads **Publish Menu**; the share message opens `🍲 *Menu: <title>*`. Tables, routes, and notification types keep the `drop` vocabulary — see `architecture.md` §4.6 for the split and why it stays. The rest of this section says "drop" when it means the record and "menu" when it quotes what a resident sees.

| Aspect | Details |
|--------|---------|
| **Purpose** | Time-gated group ordering for home chefs and food businesses — weekend specials, home baking, pop-up meals |
| **Tables** | `mcn_preorder_drops`, `mcn_preorder_items`, `mcn_preorder_orders`, `mcn_preorder_order_items`, `mcn_drop_reports`, `mcn_listings`, `profiles` |
| **Catalog rules** | Tabs: Open / Past / Mine (plus a lead-only **Review** — see Reporting & moderation), defaulting to `cutoff_at` ascending. **Filter and sort sit in that same pill row**, to the right of the tabs and styled as the same pill — icon-width, so five affordances still fit a narrow phone without the row scrolling. Both open bottom sheets. **Sort** offers Closing soon (default) · Delivery soonest · Just added · Price low→high; the pill fills primary while any non-default sort is active. A fifth option, **Most ordered, is built but parked** behind `DROP_SORT_MOST_ORDERED_ENABLED` (`constants/featureFlags.ts`) — see [docs/hidden-features/](hidden-features/README.md). **Filter** offers four groups: delivery day (today / tomorrow / this weekend / next 7 days), meal (breakfast / lunch / snacks / dinner, read from the host-chosen `meal_type`), veg / egg / non-veg, and price (under ₹100 / ₹100–300 / above ₹300, judged on the drop's **cheapest** item). The filter pill shows a count of active groups. Filter edits are held in a draft and committed on **Show results**, so the list underneath does not churn while a half-built filter set is being assembled. Filtering and sorting are client-side: a community sees a handful of open drops at a time and every field is already loaded for the tiles, so a round trip per chip tap would cost more than it saves. A drop matches a diet filter when **at least one** of its items does — you order individual items, and the menu marks each one, so a mixed drop is a real result for both Veg and Non-veg. A drop whose menu failed to load has no price and is kept rather than hidden, so a data-loading failure never masquerades as a filter result; on price sort those drops sink instead of leading a "cheapest first" list with no price to show. Filtering to nothing gets its own empty state ("No menus match these filters") with a **Clear filters** action, distinct from a genuinely empty tab. Per-drop order counts and item quantities come from `get_mcn_drop_order_counts` — a direct read of `mcn_preorder_orders` returns zero rows logged-out, since those rows carry buyer name, phone, and flat and are deliberately not public. That call is gated on `DROP_SORT_MOST_ORDERED_ENABLED`, since the parked sort is its only consumer and an ungated fetch would be a round trip per load for nothing on screen. Revenue metrics (total, completed, order count) still need the amounts, so they stay on a direct read and load only on the Mine tab. Anonymous viewers see only `status = 'open'` drops. Cards show host identity (name plus flat number when available), the linked business listing when present, and side-by-side close/delivery timing chips. **A drop with no photo falls back to a bundled illustration** (`assets/images/food-drop-placeholder.jpg`, 1200×657 JPEG / 143 KB — a drawn tiffin spread) rather than the old empty grey box reading "food photo", which looked like an unfinished screen. **Illustration, not photograph, is the deliberate choice**: a realistic thali shot on a paid listing reads as a picture of the food you are about to buy, whereas a drawing cannot be mistaken for the host's actual dish. Bundled, not remote, so it renders offline and costs no request. Real covers crop `contentPosition="top"`; the placeholder crops from the centre. The drop **detail** screen uses the same fallback so a photoless drop does not open to a blank slab of text, but renders it non-tappable — there is no real photo to open full-screen. The **"Reserve now" CTA overlays the cover photo's bottom-right** rather than taking a full-width row under the body: it keeps the affordance visible in the feed while costing the tile no height. **It is hidden for the host** — you cannot pre-order your own drop, so a shimmering CTA on your own card invites a tap that goes nowhere; hosts manage from inside the drop, and tapping the card still opens it. The card derives this from `useAuth()` rather than an `isCreator` prop, since a prop is one a caller can silently forget to pass. A soft highlight drifts across it continuously while the drop is open (`ReserveButton`) — two gradient bands running half a cycle apart, so one is always on the pill; a single band spends ~40% of its cycle clearing the edges, which reads as the glow arriving late; closed and completed drops get a muted, static "View menu" instead, since animating a dead end is just noise. The sweep is gated on `useReduceMotion()`. The cover photo shows the **top 30% of the photo's own height** (`getTopCropTileImageStyle()`, fed the natural aspect ratio expo-image reports in `onLoad`), so tile heights vary with the picture: hosts upload portrait posters, and a fixed slab of pixels showed a thin strip off the top of one. It is bounded — never shorter than `getNetworkTileImageHeight()` (~11.5% of viewport, clamped 84–130px, re-measured from `useWindowDimensions()`, itself replacing an older fixed 108px strip) and never taller than the detail hero, which is what stops a tall poster owning the whole fold. That floor is derived, not chosen: the feed must show **at least three tiles at once**, and after ~270px of fixed chrome and a ~97px compact card body there is only ~100px left for the photo. Opening the drop grows the same photo to **30%** (`getMediaHeroHeight()`, clamped 150–280px): on a tile the photo competes with the two tiles below it for the fold, on a detail screen it is what you came to look at and competes with nothing. Enlarging the tile is therefore not a free win — it buys a prettier card by pushing the third one off screen, which is the trade the crop fraction buys. It was tried at 40% and pulled back to **30%**, where a square cover sits on the `minHeight` floor and only portrait posters — the covers that actually looked cropped to a strip — grow. The same budget is why the tile carries **no description and no CTA button**: the "Reserve now" button called the identical `onPress` as the card wrapper, so it bought nothing but ~65px (divider + button — a third of the card body), and tapping the card has always opened the drop. Open/closed state still reads from the cut-off chip's colour and copy, and the description is one tap away. Three tiles fit from roughly an 800px viewport upward (iPhone 13+ ≈ 844, Pixel ≈ 890); a short 640–700px screen gets ~2.1–2.4, because the fixed chrome eats proportionally more — there is no image height that fixes that without gutting the card. The crop is `contentPosition="top"` in both places, since `contentFit="cover"` centre-crops and takes the top off a plated dish, and both are tappable into `ImageViewer`. Community business cards use the same crop token, and community events cards the fixed tile height, for the same reasons. A `?id=` param redirects into the drop detail (web deep-link bridge). |
| **Publish rules** | A drop specifies title, prep notes, fulfillment date, fulfillment time (system time picker), **meal slot**, `cutoff_at` deadline, and items with name, unit, price, **veg / egg / non-veg**, and optional `max_quantity`. The meal picker (breakfast / lunch / snacks / dinner) is **its own titled card section** headed "Which meal is this? *", placed directly after the delivery schedule card, with four full-width chips showing the label and its time-of-day hint (Morning / Midday / Evening / Night). It was previously an 11px sub-label tucked under the delivery-time fields, where hosts skipped it — being pre-filled, it looked already answered. It still **seeds itself from the delivery time** — before 11:00 breakfast, to 15:30 lunch, to 19:00 snacks, else dinner — then stops following once the host picks one themselves, so changing the time later never silently overrules a deliberate answer. Editing an existing drop treats its stored value as already chosen and does not re-guess. It is stored rather than derived at read time because the clock cannot tell a late snack from an early dinner, and residents filter the catalog on it. **The closing time and delivery time are asked for explicitly, as four separate date/time fields.** A preset-chip version ("When is the food ready?", with the cut-off derived two hours before delivery) was built and reverted on 2026-08-24 — presets hid the closing time behind a derived sentence, and a host needs to see and set both times outright. The two picker pairs now render through the shared `components/DropDateTimeRow.tsx`, which owns the `Platform.OS` branch to a web `<input>` that `@react-native-community/datetimepicker` requires; the layout, labels and card structure are unchanged. Diet is per item, not per drop, because a single menu is routinely mixed (veg curry alongside a chicken biryani) — a drop-level label would be wrong on half the listings. It **defaults to veg**, so a host who never touches the row publishes a veg menu; items created before the column existed were backfilled to veg and stay wrong until their host edits them, which ages out on its own since drops are short-lived. The same value drives the green/amber/red dot beside each item on the drop detail menu and the dots beside the title on each catalog tile (one per diet type the menu offers, ordered veg → egg → non-veg). **Delivery time must be strictly later than the cutoff, and neither may be in the past** — one implementation, `validateDropSchedule()` in [`lib/dropSchedule.ts`](../lib/dropSchedule.ts), shared by the publish form and every republish path. The date pickers refuse any day before today (the fulfillment picker's floor is the cut-off date), and submitting re-checks both timestamps against the current moment — a drop published past its own cut-off would be un-orderable, since `place_mcn_preorder` rejects every order once `cutoff_at <= now()`. In edit mode the past-time check applies only to values the host actually changed, so a drop left open past its cut-off can still be edited for a typo; **create, duplicate and republish pass `loadedSchedule: null`**, so both timestamps are checked against now. There is no overall drop-order cap — capacity is per item, and it is enforced by a database trigger, not just the UI. An item's `max_quantity` is a **total shared across every buyer's orders combined**, not a per-order allowance — e.g. `max_quantity = 5` means at most 5 units total can ever be ordered for that item across all residents. The item form and the item row on the detail screen both say so explicitly, and `get_mcn_drop_item_availability` / `check_mcn_drop_item_quantity_capacity` (plus insert/update triggers on `mcn_preorder_order_items` and `mcn_preorder_orders`) enforce it server-side. |
| **Republishing** | A host who cooks the same menu weekly no longer retypes it. On the **Mine** tab, each of the host's own past menus carries a **Republish** action in the tile's overlay slot — the one that holds "Reserve now" for everyone else and is empty for the host, so it costs the tile no height. It opens a sheet (`components/RepublishDropSheet.tsx`) showing the menu **read-only with its prices** — a menu rerun weeks later can carry stale prices, and this is the last look before the whole community is notified — and asking only for the new **pre-order closing time** and **delivery time**. Publishing lands the host on the new drop. "Edit full menu instead" escapes to the publish form via `?fromDropId=`, the same duplicate path, for the run where an item or price also changed. **Both that escape and publishing navigate before clearing the sheet's state**: the sheet owns a history entry through `useWebBackToClose`, and closing it runs `history.back()` unless something has already navigated on top — clearing first popped the host straight back off the screen they had just been sent to. `loadDraftFromDrop()` in [`lib/dropDraft.ts`](../lib/dropDraft.ts) carries title, notes, cover, meal slot, linked listing and every item (name, unit, price, description, photo, `max_quantity`, diet), and carries **nothing about when**. Orders stay with the original drop; new item rows mean `max_quantity` restarts at zero, which is correct. The sheet **counts the host's open drops up front** and disables publishing at the cap of 3, since `enforce_max_open_drops_per_host` would otherwise surface a raw Postgres exception after the host had filled the form in. **A drop hidden for review is refused** by `loadDraftFromDrop` and the action is not rendered: hiding force-closes a drop precisely so a lead can look at it, and republishing identical content would re-broadcast it (`on_drop_published` fires on every INSERT) in one tap. **A recurring-schedule version was built and removed the same day** (2026-08-24): it worked by reminding the host each scheduled morning, and Wooru ships as a web/PWA where a notification cannot be relied on to arrive — a rhythm the host trusts but that never fires is worse than no rhythm. Its table and sweep were dropped in `20260924000200`. |
...
| **Ordering rules** | Residents order before the cutoff with flat number and phone. **The phone box is asked for once, not every order.** It prefills from `profiles.phone_number`, and if the profile has none, the number typed on a successful order is written back to the profile (normalized to 10 digits via `lib/phone.ts`) so every later order arrives pre-filled — this is the only writer besides Profile → Edit and the phone-OTP signup trigger. The write is deliberately fill-only: an existing profile number is never silently overwritten from here, since Profile → Edit is the one place it changes, and the order still carries whatever they typed in `buyer_phone` either way. It also never fails the order — the pre-order RPC has already committed by then, so a rejected profile write is logged and the resident simply retypes next time. The prefill only backfills an empty box, so it cannot clobber a number typed for this order or loaded when editing an existing one. A resident with no profile number yet sees a hint under the field saying it will be saved and where to change it. Past the cutoff, new orders are blocked automatically. A resident may place **multiple** orders on the same drop while it is open. `confirmed` orders can be edited or cancelled by the buyer — editing correctly excludes the order's own prior quantity when checking remaining shared item capacity. Once the host marks an order `fulfilled`, it displays as **Delivered** and becomes immutable. The resident sees whether an order was cancelled by themselves or "Cancelled by host", along with any cancellation note provided by the host. |
...
| **Reporting & moderation** | Any signed-in non-host resident can report a drop from its detail screen (flag link, reason picker — not a food drop / spam / inappropriate / food safety concern / other, one report per resident per drop via `mcn_drop_reports`, mirroring `mcn_listing_reports`). At **3 pending reports** the drop is auto-hidden and leads are notified. A president or vice-president can also hide a drop directly from a **Community lead controls** card on the detail screen, and restore it from the same card. **Restoring returns the drop to the exact status it held before it was hidden** — hiding force-closes it to stop new orders, and the pre-hide status is stashed in `flagged_prev_status` so restoring is a true undo rather than a one-way trip to Past (nothing else in the app can reopen a closed drop). If the cut-off passed while the drop was hidden it stays in Past, on its own timing rather than because it was moderated. Restoring also dismisses the open reports, so the next report does not immediately re-hide it. **Leads find hidden drops on a fourth, lead-only "Review" tab** in the catalog — hiding removes the drop from every other tab, so without its own surface a lead would have no route back to what they just hid. Residents never see the chip. The fourth chip no longer fits beside the filter and sort pills on a narrow phone, so the lead's chip row scrolls internally and takes `flex: 1` as its slot; `ChipRowSlider`'s scrollable root is a horizontal `ScrollView`, which has no intrinsic width and collapses to nothing if dropped straight into the `flexDirection: 'row'` controls row (which is exactly why the chip did not render on the first attempt). Tiles there (and the host's own copy under "Mine") carry an amber **"Hidden for review"** badge in place of the usual cut-off chip. The moderation card also appears to a lead on **their own** drop when it is hidden — a lead never needs to hide their own drop, but three reports can auto-hide it, and in a single-president society nobody else could restore it. Moderation notifications (`drop_reported`, `drop_auto_hidden`, `drop_hidden_host`, `drop_hidden_buyer`) deep-link into the drop; the equivalent listing notices (`listing_reported`, `listing_auto_hidden`) were dead ends since 2026-08-21 and now deep-link too. **Hiding is not deleting and not a public spam badge.** A hidden drop leaves the catalogue, stops accepting new orders (enforced in the order `INSERT` policy, not just the UI), and keeps every existing pre-order intact. The notice is **scoped, never public**: the host is told it is hidden and why, existing buyers are told the drop was withdrawn and to contact the host, leads see the moderation state, and everyone else simply does not see the drop. Drop tiles carry the host's name and flat number, so a public flag would be a public accusation against a named neighbour by an elected one. The host cannot un-hide or reopen their own hidden drop — only a lead or platform admin can. |
| **Roles** | **Anonymous users can browse** — drops are publicly readable so shared links work logged-out. Login is required to order or publish. The creator manages the drop; the **manage dashboard is host-only** (a non-host reaching that URL is redirected back to the drop with a toast, since RLS returns them zero orders and an empty dashboard reads as "nobody ordered" rather than "you cannot see this"). **The creator or a platform admin can permanently delete** a drop and its items/orders — **leads cannot** (changed 2026-09-15: deleting cascades to every pre-order with no notice to buyers, which is too much reach for a one-tap action by an elected neighbour; leads hide for review instead, which is reversible and notifies everyone affected). Delete lives in a **bottom-of-screen danger zone** (`components/DangerZone.tsx`) on both the drop detail screen and the manage dashboard — it used to sit inline in the header action row beside Edit drop and Mark completed, one mis-tap from routine work, and it is the only action on those screens that cannot be undone. The zone names what is lost (drop title, pre-order count, that buyers are not notified) and carries a shared spam caution: posting and deleting repeatedly looks like spam to neighbours and is visible to the society president. Confirmation goes through `confirmAction`, so it is `window.confirm` on web and `Alert.alert` on native. |
...
| **Grade rung** | Beside the free-text class label, the form carries a **contextual grade chip row** whose options follow institution type — Playgroup/Nursery/LKG/UKG, Class 1–12, or 1st–5th Year. The pick is stored as a number so entries can be compared; the free text still owns display (parents want to keep the section, "8 - B"). The rung is read automatically from whatever the parent types until they tap a chip, and is re-read when institution type changes (a Class 8 rung is meaningless once the entry becomes pre-school). Grade sorting in the directory uses the rung where present and falls back to label comparison for rows that could not be parsed, which sort last. |
| **Matching** | Intents are matched, not just filtered. On save — and on demand from **Find matching parents** on your own card — a sheet lists neighbours who asked for one of the same things, best first. Per-intent rules decide what counts: carpool / study group / homework help / school info require the **same school**; playdate and sports/activities require a grade **within one year** (falling back to same-school when a rung is unknown); `other` accepts either. All matches are pre-selected, and **Notify** sends each of them a `parent_corner_match` alert. Contact details are deliberately absent from the sheet — the neighbour gets the notification and chooses whether to reply, so neither side has to make a cold call. Skipped entirely when no intent was picked. |
| **Roles** | Residents manage their own entries. **Owner or lead** can edit or delete any entry. Editing another resident's entry via URL checks ownership and returns to directory if unauthorized. Only the **owner** sees Find matching parents and can notify — a lead cannot nudge on someone else's behalf. |
...
| **Catalog rules** | Merges the 81 curated regional schools with community-submitted ones; the subtitle prints the live total rather than a hardcoded "50+". Cards show syllabus, level, distance, fee range, and a parent review count — counted from `school_reviews` for curated and community-added schools alike, since only the latter carry a stored `review_count`. Up to **3 schools** can be selected for side-by-side comparison at `/mcn/schools/compare?ids=a,b,c`; selecting a fourth is refused with an info toast, and the comparison columns follow the order they were ticked in. |
| **Filter chips** | Board and locality chips match on a **keyword list**, not on their own label, and match whole words — `Cambridge` finds `CBSE / Cambridge` and `CBSE, Cambridge (IGCSE)` alike, `IB` finds the school whose board is the bare string `IB` without `ICSE` matching `IGCSE`, and `Ramachandrapuram` also covers `R C Puram`. The lists live in `constants/schoolCatalog.ts` and are shared with the add form, so a board a resident picks is always a board they can filter by. |
...
| **Add school rules** | Required: name, distance (0–999.9 km — the column is `NUMERIC(4,1)`, so a larger value is rejected in the form rather than surfacing as a bare "failed to add"), fee range. Optional: area/locality (chips from the same list the catalog filters by, plus free text), address (what produces the Maps link when no map URL is known), facilities, description, website, and phone — which, when supplied, must be 10 digits. |
| **Detail view refresh** | The screen refetches on **focus**, so a report card submitted at `/mcn/schools/review` is on the page when the parent lands back on the school. |
...
|--------|---------|
| **Tables** | `mcn_posts` |
| **Rules** | Title required (max 80), description optional (max 280), contact info **mandatory** (max 80) — a borrow post nobody can respond to is a dead end. A contact that is purely a 10-digit phone number is normalized to bare digits so it can become a WhatsApp link; anything else ("knock on A101") is kept verbatim. The composer always writes `kind = 'borrow'`: `mcn_posts.kind` still allows `'business'`, but community businesses moved to `mcn_listings` and nothing renders a business-kind post, so writing one would create an invisible row. |
| **My Submissions** | The Borrow tab lists **only the signed-in resident's own** borrow posts (filtered by `user_id` *and* `kind`), grouped into Active and Closed with close/delete actions. It is not a community feed — and because it is not, the hub card's badge counts the resident's own active posts rather than the whole community's, so the number and the destination agree. |
...
| **Rules** | Section order is fixed: **events ("Happening soon") → funds → manage rows (lead only) → residents/SOS tiles → community info**. The tile row holds **three** tiles and wraps two-up: **Emergency numbers and Blood donors sit side by side**, with Residents directory full-width beneath them — the two urgent surfaces share a row so neither is mistaken for the other, and the directory is the one you go looking for rather than reach for in a hurry. Emergency and blood donors were a single "Emergency & donors" tile until 2026-08-25; residents read it as one emergency feature and never found the donor registry, so each half now gets its own tile and opens `/sos` with `?segment=emergency|donors`. The events section shows up to 5 upcoming published events in a horizontal carousel sorted by date, above funds — timely content earns the fold ahead of a funds summary that reads ₹0 for most communities. The hero shows the community name and an Invite button on its right; the "going around the building" pulse line was intentionally removed. That hero button fires the same share action as the one on the community-info panel — deliberately duplicated because the panel sits below the fold and residents were missing it. Community info includes a join-code tile with an Invite-neighbors share action; the shared message includes `/api/share-community?id=…` (`api/share-community.ts`) so WhatsApp renders a branded preview card (`get_community_og_card(p_id)` RPC — `communities` has no anon SELECT policy, and the join code itself is never put in the RPC response, only in the plain-text message). The funds request CTA exists only in this section. |
...
| **No president yet** | When `communityHasLead` is false, a notice card sits directly under the hero: "Funds not active yet", stating "Funds and block in-charges switch on once a president or vice president is appointed.", with a link to the residents directory. The heading names what is waiting rather than who is missing — the old "No president yet" read as a complaint about the society to every resident who opened the tab. The funds section replaces its "Request funds support" CTA with "Funds aren't active in this community yet. They'll switch on once a president or vice president is appointed." See `architecture.md` §3 → Leaderless communities for what is and is not gated. |
...
| **Lifecycle** | Platform admins seed blocks at community-approval time (setting `blocks_enabled = true` and the label) and maintain them thereafter. Joining residents pass through `/community-join-block`. Profile shows a block picker only while blocks are active. Contribution flows load the collection grid through `list_collection_targets_for_collector(...)` so block in-charges see only flats in their own block. (Its predecessor `list_eligible_contributors_for_collector(...)` was resident-based rather than flat-based; it still exists in the database but no screen calls it.) |
...
| **Entry points** | Two tiles on the Community tab, and the screen renders **one surface at a time** — `?segment=donors` gives the blood-donor registry, anything else the emergency directory. The in-screen segmented control was removed on 2026-08-25: once each half has its own tile, a toggle underneath re-merges what the tiles just separated, which is the confusion the split was meant to fix. `activeSegment` is therefore derived from the param, not state. Header title and subtitle follow it, and only that surface's data is fetched — the donor half skips the `emergency_contacts` read and vice versa. Switching surfaces means going back to Community, which is deliberate: these are two errands, not two views of one. |
...
| **Rules** | Active residents only, grouped by block when `blocks_enabled`. **No resident ever sees another resident's email address — a president or vice president included** (2026-08-17). Rows carry name, flat, role badge, and — for leads and platform admins only — phone number. Phone is retained as the lead's contact channel because the rest of the app already reaches residents that way (WhatsApp links, order contacts). See `architecture.md` §4 → Resident email is not readable for how this is enforced, which is column grants rather than UI. Leads open a resident sheet and can remove non-lead residents (via `confirmAction`, so it works in the PWA). Role badges render President / Vice President / Resident. When the community has no lead at all, a notice strip above the list says so — otherwise residents have to infer it from the absence of a badge on every row. Accepts `?returnTo=community\|profile`. |
...
| **Rules** | Treasurers manage collectors and all transactions; collectors add contributions only; residents are view-only. Leads are treated as treasurer-level. **Exactly one treasurer per fund** (enforced by migration `20260813000000` and `fund_role_guard`). In block-enabled communities, assigning a collector requires choosing a block — there is no all-residents option in that flow. **The two long ledgers are not on this screen** — they moved to `/funds/contributions` and `/funds/expenses` on 2026-09-21 and that entry owns their grouping and filter rules; this screen carries the summary, the breakdowns, role management, and a link row to each. Shows collection coverage badge `{n} of {totalFlats} flats collected` for progress tracking — general contributions only, so an "other contribution" cannot push a block past 100%. A **Collected for** card sits above the block table whenever the fund holds money under more than one heading, listing the general contribution and each free-text purpose with its entry count and subtotal; it is ungated for the same reason the block table is. Leads can mark a fund **closed** (`is_closed`), blocking further transactions and edits. If funds are inactive, stale links render a safe inactive state instead of loading ledger actions. |
| **Block-wise Collection** | A card between the action buttons and the role-management sections listing every block with `{paid}/{total} flats` and the amount it has collected, plus an "Other" row when any money has no block to sit under — other contributions (which carry no flat key at all), outside sponsors, and flats missing from the inventory. **Visible to every resident, outside all permission gates** — checking your block's numbers should not require asking the treasurer. Computed client-side from `list_community_flats` and the income rows, both of which every approved community member can already read; renders only where the community has named blocks. |
| **Export** | An **Export** pill sits to the right of the "You are a …" line and writes the whole ledger to CSV via `lib/csvExport.ts`: fund/status/timestamp, the three totals, the by-purpose table (when the fund has more than one heading), the block-wise table, every contribution (block, flat, contributor, what it was for, resident-or-other-or-sponsor, method, collected-by, amount, ISO date) in on-screen order, then every expense. Amounts are bare numbers so the column sums in a spreadsheet. **Shown to everyone** — it is the same data the screen already renders to every resident. CSV, not `.xlsx`: `xlsx` is a dependency but only `scratch/` build scripts use it, and pulling SheetJS into the client bundle to produce a file Excel opens from CSV anyway is not worth a few hundred KB on mobile data. Web downloads via `Blob` + `<a download>`; native writes to the cache dir and opens the share sheet. A UTF-8 BOM is prepended or Excel mangles non-ASCII names, and cells are RFC 4180 quoted — real contributor names contain commas. |
...
Every contribution for one fund, **grouped by block and ordered by flat number inside each block** (`groupContributionsByBlock` in `lib/fundLedger.ts`) — the same `length(flat_number), flat_number` order the collection grid uses, so ground-floor numbers (G1) precede 102 and a flat sits in the same place on both screens. Each block header carries its own entry count and subtotal. Rows with no block — **other contributions** (which carry no flat key at all), outside sponsors, a member in a community with no flat inventory, a flat archived after it paid — fall into a trailing "Other contributions" group; when no block has contributed at all the headers are suppressed and the list reads flat. Names and flat labels resolve from the stored snapshot columns (`contributor_name`, `contributor_flat_id`, and `contributor_flat_label` for ad-hoc rows), with legacy fallback to profile mappings. A **payment-method filter** — All / Cash / Online / Not recorded, each chip carrying its count — narrows the list. A second **purpose filter** appears beneath it once the fund holds money under more than one heading: All, General contribution, then a chip per free-text purpose, each with its count, derived from the rows themselves rather than a catalog. The tally line shows the filtered count and total, or — unfiltered — how many *flats* have paid their share, which counts general contributions only. Rows show contributor, flat, `For <purpose>` on other contributions, method, collected-by and date, and are tappable for anyone who may edit them. Group headers count **entries**, not flats, since a block can now hold more rows than it has flats. Requires sign-in.
...
| **Rules** | Contribution mode uses the block-aware collection target list (grouping flats by floor with quick search by flat/occupant name) and disables flats that have already contributed. A block-scoped collector only sees flats of their assigned block; a collector with no block, the treasurer, and leads see every flat. Prefills occupant name in an editable Payer name input field. Saves flat ID, user ID (if signed up), and snapshot name. Expense mode requires title and amount and never sets contributor columns. A **Payment method** Cash/Online toggle sits under Amount in both modes and defaults to Cash (which is how a collector walking the block is actually handed money); editing an older row leaves the toggle at its default rather than asserting the row was cash, so saving is what records the real answer. Supports an optional receipt image (`event_transactions.image_url`). Tapping an existing contribution or expense row reopens this screen pre-filled for editing, resolving historical payer details off the transaction snapshot columns. Inactive funds render a graceful error state. |
| **Amount** | At most 2 decimal places and at most **₹10,00,000 per entry**, checked in the form and enforced by the ledger trigger. |
| **Payer mode** | Contribution mode opens on a two-up segmented **General contribution · Other contribution** control, hidden when editing an existing row (whose mode is fixed by what it already is). Two tabs rather than three, so the labels can say what they mean instead of being clipped to fit a phone. |
| **Other contribution** | For money handed over for something specific — food, the god idol, prasadam, or a shop paying for the lighting — by someone the collector identifies by name rather than by flat. Fields: contributor name (required), flat (optional, free text), phone (optional), **Contributing for** (required, free text, with one-tap suggestion chips from `SUGGESTED_PURPOSE_LABELS`), plus the shared amount / method / notes. It deliberately picks **no flat**, so it never marks a flat as paid and never collides with the one-contribution-per-flat rule; the flat field is a note about where the money came from, and the phone is never rendered into the contributions list every resident can read. Any collector, treasurer, or lead may record one, and no block prompt appears. |
| **Outside sponsor (retired from the form)** | The third **Outside sponsor** tab was removed once Other contribution existed: a collector wanting to log "Sharma Electricals — for the lighting" just typed it there, so the president/VP-only rule on sponsor rows was guarding a door with no wall beside it, while still handing collectors an error they could not resolve. Outside money is now an other contribution with the business named as the contributor; the optional phone field replaces `sponsor_phone`. **Existing sponsor rows are untouched** — they still read "Outside sponsor" on the Contributions list, still reopen here in sponsor mode with name/phone/note, and are still lead-editable only. The database shape and its trigger rule remain; nothing creates new ones. |
| **Notice card** | The info card above Amount renders only when it has something to say — editing, an expense, a permission refusal, or Other-contribution mode. Recording a flat's share shows no notice: the tab, the flat grid, and the PAID badges already say it, and an empty bordered box reads as a rendering bug. |
...
**Mobile number** is editable here and is the only place it can be changed. It is optional; a non-empty value must be a valid Indian mobile and is normalized to 10 digits (`lib/phone.ts`) before saving, and clearing the box writes `null`. Most residents never type it here — it is captured automatically the first time they order from a food drop (§4.3) — so the field carries a hint saying what it prefills.

**It is a saved contact number and nothing more.** It is not an identity, not a credential, and **not wired to phone login** in either direction: sign-in resolves an account from the synthetic `phone_91…@auth.wooru.in` address, never from this column, so editing it here cannot affect anyone's ability to log in. Treat it as unverified — nobody has proved they own it. (Phone-OTP signups happen to arrive with it pre-filled by `handle_new_user`, which is just a convenience, not a link.) If a flow ever needs a *trusted* number, the number saved here is the thing to send an OTP to — verification would be added on top, not assumed.
...
| **Rules** | Users mark individual rows or the whole list as read. Mute toggles in Pre-order Food and Parent Corner headers allow users to suppress `food_drops` and `parent_corner` broadcasts without losing transactional pre-order alerts. The control is a **channel-named label beside a switch track** — "Food alerts" / "Post alerts" (`components/MuteToggleButton.tsx`) — deliberately *not* a bell in a circle — that is the shape of `NotificationBell`, which navigates to the notification list, and a header-right bell reads as "open my notifications" rather than "mute this section". It renders only when signed in, since a preference needs a user to belong to. Funds-activation types (`funds_access_requested`, `funds_access_approved`, `funds_access_rejected`, `community_lead_appointed`, `funds_access_revoked`) are handled alongside the core flows; legacy promotion and admin-review payloads are still recognized so old rows stay tappable; unknown types fall through safely. |
...
| Assign treasurer | ❌ | ✅ | — | — |
| Assign collectors | ❌ | ✅ | ✅ | — |
| Add general contribution (a flat's share) | ❌ | ✅ | ✅ | ✅ own block |
| Add other contribution (named purpose, no flat) | ❌ | ✅ | ✅ | ✅ |
| Edit existing contribution | ❌ | ✅ | ✅ | ✅ |
| Create an outside-sponsor row | ❌ | ❌ | ❌ | ❌ |
| Edit an existing outside-sponsor row | ❌ | ✅ | ❌ | ❌ |
| Add expense | ❌ | ✅ | ✅ | ❌ |
| Close / reopen fund | ❌ | ✅ | — | — |

Creating a sponsor row is ❌ for everyone because the **Outside sponsor tab was retired from the collection form** — outside money is now recorded as an other contribution naming the business. The row shape, its president/VP-only trigger rule, and lead-only editing all remain for rows written before that, so the "edit" line above is still live. See [`disabled-features.md`](disabled-features.md) §Removed.
...
| Report a drop | any but own | — | — | — |
| Hide / restore a drop for review | ❌ | ✅ any but own | — | — |
| Delete drop | own only | own only | — | — |
| **MCN — carpools** |
| Browse rides | ✅ | ✅ | — | — |
| Publish ride | ✅ | ✅ | — | — |
