...
| **Business rules** | Required: title, category, provider context, and date. Categories are drawn from the full shared list in `constants/categories.ts` and presented through the same two-level grouped picker (Home Support, Transport & Vehicle Care, Repairs, Personal, Events, Other) used on the Add Provider form. Users can link an existing provider (label: "Select existing provider") or enter a manual provider name and phone. Manual phone and WhatsApp values accept flexible formats but are validated and normalized to 10-digit mobile numbers before save. Visit dates are stored as local calendar dates (`YYYY-MM-DD`) to avoid timezone rollover into the previous day. New visits start as `upcoming`. |
