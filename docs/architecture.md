...
5. **Service worker** — registered from `APP_SHELL_HEAD` (app pages) and from `public/landing.html` (the root). Registration checks `document.readyState` for `complete`/`interactive` so it still runs when the bundle loads after the load event.
