...
| Fund modal | Collected / Spent / Balance / **Coverage %**, per-block collection coverage bars, treasurer, collectors, and the **full ledger** (`platform_get_fund_ledger`) with a running balance and CSV export. The ledger separates `resident_contribution`, `other_contribution`, `sponsor_contribution`, `other_income` and `expense`, and carries `purpose_name` — the free-text "what it was for" on an other contribution — as its own column in both the table (rendered after the payer, as `Ramesh (A-207) · Prasadam`) and the CSV. `contributor_name` and `contributor_flat` fall back to the row's own snapshot columns when the `profiles` join finds nothing, so ad-hoc donors and unregistered flat payers are no longer rendered as a nameless "Resident". `platform_get_community_funds`'s `contributions` array makes none of these distinctions and still does render sponsor income as a nameless "Resident". |
| Coverage figures | `platform_get_fund_collection_coverage` counts **distinct residents**, not transaction rows. Two bugs were fixed here on 2026-08-27: `residents` used `COUNT(p.id)` across a join that fans out once per transaction, and `contributors` counted income rows. Neither could over-report while a resident had at most one income row per fund; both would have the moment other contributions existed. The RPC is only correct because ad-hoc rows carry no `contributor_user_id` — **never give an other contribution a real member or flat key**, or every coverage bar in this console inflates. |
...
### Reading `profiles` from the console

Never `from('profiles').select('*')` or name `email` in a select here. `20260918000000` revoked table-level `SELECT` on `profiles` and re-granted a column list that excludes `email`; because a column-grant miss fails the whole statement, both patterns return `permission denied for table profiles` rather than degrading. Use `platform_get_community_residents(p_community_id)` for a community's roster and `platform_get_profiles_contact(p_ids)` to resolve names/phones/emails by id. The remaining direct reads ([`funds-requests.js`](../admin-dashboard/js/funds-requests.js), [`auth.js`](../admin-dashboard/js/auth.js)) are safe only because they name granted columns — adding `email` to either re-breaks that screen.
