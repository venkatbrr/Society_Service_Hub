...
| PWA Service Worker push handler rollout | Installed PWAs cache the service worker and update stale-while-revalidate. New push handler requires bumping `CACHE_NAME` (`wooru-pwa-v11`) and opening the app twice or performing a long pull-to-refresh (`HARD_RELOAD_THRESHOLD`). |
