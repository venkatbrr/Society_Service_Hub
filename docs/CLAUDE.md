...
- **Replacing is right** for post-save redirects, redirect bridges (`/mcn/drops?id=…`, `/mcn/drops/manage`), and sibling-tab toggles (drops ⇄ business). A bridge that uses `push()` creates an infinite back loop. **But call `replaceTracked(router, route)`, never `router.replace()` directly** — see the trap below.
...
| Calling `router.replace()` directly instead of `replaceTracked(router, route)` | `replace()` removes an entry from real browser history, but `syncNavigationStack` only sees "the pathname changed" and *pushes* — leaving the departed route in the tracked stack as a phantom. `getPreviousRoute()` then names an unreachable screen, so `goBackSmart()` stops matching the parent and takes its `replace()` fallback instead of popping. Result: a burned history slot, two identical adjacent entries, a dead forward button, and **a back press that appears to do nothing**. Fixed across the create-then-detail flows on 2026-08-09. |
