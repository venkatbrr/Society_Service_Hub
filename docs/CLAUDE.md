...
| Edits not showing up on the dev server after a refresh | Almost never Metro. The PWA service worker used to register on `localhost` too, and its static-asset branch is **cache-first with no eviction until `CACHE_NAME` changes** — so it pinned Metro's bundle and the browser stopped asking the dev server at all. Registration is now skipped on `localhost`/`127.0.0.1` (`app/+html.tsx`); append `?sw=1` to force it when testing install or push locally. A worker registered before that guard landed **keeps running** — unregister it once via DevTools → Application → Service Workers, or `navigator.serviceWorker.getRegistrations().then(r => r.forEach(x => x.unregister()))`. To tell the two apart, curl the bundle out of Metro and grep it for your new string; if it is there, the problem is in the browser. |
...
| Returning `false` from the Android `hardwareBackPress` handler in `useSyncedBackNavigation` when `canGoBack()` is true, to defer to React Navigation's own listener | `/mcn` is a Stack nested under the root Stack, and React Navigation's hardware-back handling has a known upstream bug with that shape (github.com/expo/expo/issues/33489) — deferral can pop the wrong navigator and drop straight to the root instead of the previous nested screen, across every `/mcn/*` module (food drops, carpools, parent corner). Fixed 2026-08-15: call `backTracked(router)` and `return true` instead of deferring. |
| **Web: browser-back from a nested `/mcn/*` screen lands on `/network` (or `/login` if signed out) instead of the previous screen — one level in, not just deep-linked** | **Confirmed 2026-08-16, fixed the same day — upstream bug, app-level correction.** Reproduced with Playwright: cross from a *different* root-Stack sibling (e.g. `/network`, `/legal`) into `/mcn/*` with `router.push()`/`router.navigate()` (mounting the `mcn` Stack fresh this session), then push once more inside it (e.g. drop list → detail). The raw DOM history is briefly correct after one browser-back (`window.location` genuinely reads the parent URL for ~1 render), then `expo-router`'s web `useLinking.js` popstate handling independently resolves the app to its default/initial route (`/network`) ~180ms later, with no new `popstate` and nothing in this app requesting it — visible via `usePathname()`, not a real second history entry. `lib/navigation.ts`'s tracked-stack reducer is *not* the cause — it only informs `goBackSmart()` (the header arrow), never the real browser-back destination, and this reproduced even with that hook's Android branch irrelevant (web-only bug). A cold, direct page-load into `/mcn/drops` (no cross-branch push) does **not** exhibit it. `router.navigate()` in place of `router.push()` for the cross-branch hop does not avoid it either (tested). **Fix**: `syncNavigationStack()` in `lib/navigation.ts` now arms a one-shot guard (`lastPopLandedRoute`/`lastPopLandedAt`) whenever it resolves a genuine pop, and if the very next pathname change is unexplained (no new `popstate`, no declared `*Tracked` intent) and lands specifically on `POST_AUTH_LANDING_ROUTE` within `SPURIOUS_DRIFT_WINDOW_MS` (600ms — measured drift is ~180-300ms), it self-corrects with one `replaceTracked()` back to the real destination. This does **not** reintroduce the popstate-racing bug from the row above: it only acts after expo-router's own (wrong) resolution has fully committed, never mid-pop. Verified with Playwright for both the initial pop and a second consecutive back. Known false-negative: testing anonymously, the app's own `!session` guard in `app/_layout.tsx` also reacts to the transient `/network` render and can win the race with its own redirect to `/login` — not a bug in the fix, just means an authenticated session (the real-world case, where `/network` isn't guarded) is needed to see the corrected outcome land cleanly. |
...
| PWA Service Worker push handler rollout | Installed PWAs cache the service worker and update stale-while-revalidate. New push handler requires bumping `CACHE_NAME` (currently `wooru-pwa-v12`) and opening the app twice or performing a long pull-to-refresh (`HARD_RELOAD_THRESHOLD`). |
