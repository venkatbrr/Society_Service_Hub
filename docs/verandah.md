...
**Interaction-handle rule (added 2026-08-24).** Every step of an endless loop must pass **`isInteraction: false`**. `isInteraction` defaults to `!useNativeDriver`, and `react-native-web` has no native driver — so on web a JS-driven `timing` opens an `InteractionManager` handle for its whole run, and a loop never releases it. `InteractionManager.runAfterInteractions` then never drains, which is the queue `VirtualizedList` schedules its render-window growth on (`Batchinator` → `runAfterInteractions` → `requestIdleCallback`). The result is not a dropped frame — **every `FlatList` in the app stops dead at `initialNumToRender`**, and because React Navigation keeps a visited tab mounted, one visit to the MCN hub froze all of them for the rest of the session. `Animated.delay()` carries the same default and gives you nowhere to pass the flag: write it as a `timing` with `duration: 0` and a `delay` instead. Ambient motion is decoration, never a user interaction.

If you add an always-on animation, animate transform/opacity only, drive it natively where a native driver exists, pass `isInteraction: false`, and gate it on `useReduceMotion()`.
...
- **`components/PreorderDropCard.tsx` → `ReserveButton`**
  ...
  - *Overlay ring (2026-08-27)*: the pill sits on the host's own cover photo, so it has no reliable contrast behind it — a dark menu card swallowed the dark fill outright, and accent `Republish` was invisible on it. `reserveBtn` therefore carries a 1px `rgba(255, 255, 255, 0.72)` ring (a raw rgba, not a token: the `Verandah.border*` family is ink-on-light and would be the wrong direction here), and `reserveBtnMuted` overrides it with `Verandah.borderHair` because a white ring vanishes into a cream fill. Do not drop the ring when restyling — the shadow alone does not carry on a photo.
