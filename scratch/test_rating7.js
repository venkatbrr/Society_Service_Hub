...
async function run() {
  ...
  const { data: commId } = await supabase.rpc('get_user_community_id');
  console.log("get_user_community_id result:", commId);
  ...
  const { data: selectData, error: selectError } = await supabase.from('ratings').select('*').eq('user_id', user.id);
